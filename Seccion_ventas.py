...
class SeccionVentas(ctk.CTkFrame):
    def __init__(self, master, rol):
        ...
        self.titulo_label = ctk.CTkLabel(self, text="💰 Punto de Venta", font=("Roboto", 28, "bold"), text_color=PALETA["texto"])
        self.titulo_label.pack(pady=20)

        self.tabview = ctk.CTkTabview(self, segmented_button_fg_color=PALETA["sidebar"], segmented_button_selected_color=PALETA["botones"])
    ...
    def configurar_pestaña_venta(self):
        ...
        ctk.CTkButton(self.frame_datos, text="➕ Añadir al Carrito", fg_color=PALETA["botones"], command=self.añadir_al_carrito).pack(pady=20, padx=20, fill="x")
        ...
        self.btn_finalizar = ctk.CTkButton(self.frame_carrito, text="✅ Procesar Pago", fg_color=PALETA["exito"], font=("Roboto", 14, "bold"), command=self.finalizar_venta)
        self.btn_finalizar.pack(pady=10, padx=20, fill="x")

        if self.rol in ["encargado", "administrador"]:
            ctk.CTkButton(self.frame_carrito, text="🏷️ Aplicar Descuento", fg_color=PALETA["hover"], command=self.aplicar_descuento).pack(pady=5, padx=20, fill="x")

        ctk.CTkButton(self.frame_carrito, text="🗑️ Vaciar Todo", fg_color=PALETA["peligro"], command=self.limpiar_carrito_total).pack(pady=(0, 15), padx=20, fill="x")

    def configurar_pestaña_historial(self):
        self.frame_historial_principal = ctk.CTkFrame(self.tab_historial, fg_color="transparent")
        self.frame_historial_principal.pack(fill="both", expand=True)
        self.scroll_historial = ctk.CTkScrollableFrame(self.frame_historial_principal, fg_color=PALETA["sidebar"])
        self.scroll_historial.pack(fill="both", expand=True, padx=20, pady=10)
        ctk.CTkButton(self.frame_historial_principal, text="🔄 Refrescar", fg_color=PALETA["botones"], command=self.actualizar_historial_pro).pack(pady=10)

    def sugerir_productos(self):
        nombre = self.entry_buscar_nombre.get().strip()
        if len(nombre) < 2: return
        try:
            ...
            if res:
                ...
                self.entry_id_producto.delete(0, 'end'); self.entry_id_producto.insert(0, str(res[0]))
        except: pass

    def añadir_al_carrito(self):
        id_p, cant = self.entry_id_producto.get().strip(), self.entry_cantidad.get().strip()
    ...
    def finalizar_venta(self):
        ...
        total = sum(item['subtotal'] for item in self.carrito)
        fecha_actual = datetime.now().strftime('%Y-%m-%d') # <--- FECHA PARA EL REPORTE

        if not messagebox.askyesno("Confirmar", f"¿Venta por ${total:.2f}?"): return

        try:
            conn = conectar_bd(); cursor = conn.cursor()
            # INSERTAMOS TAMBIÉN LA FECHA EXPLÍCITAMENTE
            cursor.execute("INSERT INTO Ventas (Cliente, Total, Metodo_Pago, Fecha) VALUES (?, ?, ?, ?)", 
                           (cliente, total, metodo, fecha_actual))
            id_v = cursor.lastrowid

            for it in self.carrito:
                cursor.execute("INSERT INTO Detalle_Ventas (ID_Venta, ID_Producto, Cantidad, Precio_Unitario, Subtotal) VALUES (?, ?, ?, ?, ?)",
                               (id_v, it['id'], it['cantidad'], it['precio'], it['subtotal']))
                cursor.execute("UPDATE Productos SET Stock = Stock - ? WHERE ID_Producto = ?", (it['cantidad'], it['id']))

            conn.commit(); conn.close()
            messagebox.showinfo("Éxito", "Venta guardada.")
            self.carrito = []; self.actualizar_vista_carrito(); self.actualizar_historial_pro()
        except Exception as e: messagebox.showerror("Error", f"Fallo en BD: {e}")

    def actualizar_historial_pro(self):
        for w in self.scroll_historial.winfo_children(): w.destroy()
        try:
            conn = conectar_bd()
            df = pd.read_sql_query("SELECT * FROM Ventas ORDER BY ID_Venta DESC LIMIT 20", conn)
            conn.close()
            for _, r in df.iterrows():
                f = ctk.CTkFrame(self.scroll_historial, fg_color=PALETA["fondo"])
                f.pack(fill="x", pady=2, padx=5)
                ctk.CTkLabel(f, text=f"#{r['ID_Venta']} | {r['Cliente']} | ${r['Total']:.2f}").pack(side="left", padx=15)
        except: pass

    def quitar_item(self, index): self.carrito.pop(index); self.actualizar_vista_carrito()
    ...
    def aplicar_descuento(self):
        ...
        try:
            ...
            for item in self.carrito:
                base = item['precio_base']
                item['precio'] = base * (1 - descuento / 100)
